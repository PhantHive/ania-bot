                ...
                if (member) {
                    ...
                    if (role) {
                        ...
                        await message.reply(`Je n'ai pas trouvé le rôle: **${promoToRole[data.promo]}** dans ce serveur: **${guild.name}**\n`
                            + `Merci de contacter un administrateur pour qu'il ajoute le rôle: **${promoToRole[data.promo]}**`);
                    }
                }
            }
        }
    } catch (error) {
        await message.reply('**Une erreur est survenue lors de l\'auto-check pour tes rôles !**\n' +
